...
def axs_plot(data_path, data_idx, column_nr, include_dev_points=False, include_header=True, axs=None, color_coord=True):
	...
	color = color_map.get(Names[data_idx].split('/')[1], "black")


	if "Tau" in y_header:
		y_title = 'Effective correlation (ns)'
	elif "exp" in y_header:
		y_title = parameter + ' experimental values'
		...
		if 'R1' in y_header or 'R2' in y_header:
			y_title = parameter + ' (1/s)'
		else:
			y_title = parameter
	...
	df.plot(x=x_header, y=y_header, ax=axs, label=label, marker='o', linestyle="-", lw=3.0, markersize=5, color=color)

	if include_dev_points==True:
		try:
			max_points_x = [ x_list[i] for i in dev_idx ]
			max_points_y = [ y_list[i] for i in dev_idx ]
			axs.scatter(max_points_x, max_points_y, color='black', s=50)
		except:
			pass
	...
	if axs:
		...
		axs.set_xlabel('Residue number', fontsize = 18 if "results" not in data_path[data_idx] or not color_coord else 25)
		axs.set_ylabel(y_title, fontsize=18 if "results" not in data_path[data_idx] or not color_coord else 25)

		axs.tick_params(axis='both', labelsize=16 if "results" not in data_path[data_idx] or not color_coord else 20)
		...
		if "results" not in data_path[data_idx]:
			try:
				...
				axs.set_title(name, fontweight='bold', fontsize=18)
	...
	if "exp" in y_header and "results" not in data_path[data_idx]:
...
def axs_plot_avg(data_path, data_idx, column_nr, include_dev_points=False, include_header=True, axs=None, color_coord=True):
	...
	if "hetNOE" in y_header:
		...
		if "ICL2" in SIM_DIR:
			axs.set_xlabel('Residue number', fontsize = 30)
		else:
			axs.set_xlabel("")
...
def calculate_coil_probability(traj_file, top_file, stride=2):
    """
    Calculate and plot coil probability for each residue in a molecular dynamics trajectory.

    Parameters:
    traj_file (str): Path to the trajectory file (e.g., .xtc file).
    top_file (str): Path to the topology file (e.g., .gro file).
    stride (int, optional): The stride (interval) of frames to load. Default is 2 (every 2nd frame).
    ...
    coil_counts = np.sum(dssp == 'C', axis=0)  # Count 'C' (coil) residues across frames
    total_sampled_frames = dssp.shape[0]  # Total number of frames analyzed
    coil_probabilities = coil_counts / total_sampled_frames  # Calculate coil probability
    ...
    # Calculate average coil probability
			...
			if ff in item:
				...
				if rep_name + "/" + ff in Best_cases:
					if "R2" in output:
						...
						autoAxis = axs[i, j].axis()  # [xmin, xmax, ymin, ymax

						rec = Rectangle((autoAxis[0] + autoAxis[0]*35, autoAxis[2]- autoAxis[2]*25), 
							(autoAxis[1] - autoAxis[0])*1.4, 
							(autoAxis[3] - autoAxis[2])*1.4, 
					...
					if "R1" in output:
						...
						rec = Rectangle((autoAxis[0] + autoAxis[0]*30, autoAxis[2]- autoAxis[2]*5), 
							(autoAxis[1] - autoAxis[0])*1.4, 
							(autoAxis[3] - autoAxis[2])*1.4, 
							fill=False, linewidth=2, edgecolor='black')
						axs[i, j].add_patch(rec)
					...
					if "NOE" in output:
						autoAxis = axs[i, j].axis()  # [xmin, xmax, ymin, ymax]
						rec = Rectangle((autoAxis[0]+autoAxis[0]*25, autoAxis[2]+autoAxis[2]*0.25),
							(autoAxis[1] - autoAxis[0])*1.4,
							(autoAxis[3] - autoAxis[2])*1.4,
							fill=False, linewidth=2, edgecolor='black')
						axs[i, j].add_patch(rec)
