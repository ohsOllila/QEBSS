...
create_distance = os.path.join(BASE_DIR, "simulation_scripts", "PY_scripts", "calc_distancemaps_misha.py")
distance_script = os.path.join(BASE_DIR, "simulation_scripts", "PY_scripts", "plot_distancemaps_misha.py")
...
def avg_csv(input):
	dataframes = [pd.read_csv(file, header=None) for file in input]
...
def secondary_structure_bar(GRO, XTC):
	...
	initial_secondary_structure = md.compute_dssp(traj[0], simplified=True)[0]
	final_secondary_structure = md.compute_dssp(traj[-1], simplified=True)[0]
...
def contact_map_avg(input, output):
	...
	im = ax.imshow(matrix, cmap='jet', vmin=0, vmax=1, origin='lower')
...
def create_avg_distances_plot(input_file, output_file):
	matrix = avg_csv(input_file)  # Ensure avg_csv returns a NumPy array or a DataFrame

	fig, ax = plt.subplots()

	vmin = np.floor(np.min(matrix) / 10) * 10
	vmax = np.ceil(np.max(matrix) / 10) * 10

	im = ax.imshow(matrix, cmap='jet', vmin=vmin, vmax=vmax)

	ax.set_title("Average CA Distances", fontsize=18)
	ax.tick_params(axis='both', which='major', labelsize=14)

	num_xticks = len(matrix[0]) // 5
	num_yticks = len(matrix) // 5
	ax.set_xticks(np.arange(0, len(matrix[0]), max(1, num_xticks)))
	ax.set_xticklabels(np.arange(1, len(matrix[0])+1, max(1, num_xticks)))
	ax.set_yticks(np.arange(0, len(matrix), max(1, num_yticks)))
	ax.set_yticklabels(np.arange(1, len(matrix)+1, max(1, num_yticks)))
...
create_avg_distances_plot([glob.glob(SIM_DIR + i + "/md*mdmat.csv")[0] for i in Best_cases], best_cases_folder)
...
def run_pymol_operations():
	...
	for i in pdb_data:
		if i.split('/')[-3]+ "/" + i.split('/')[-2] in Best_cases:
			...
			if len(md) > 0 and len(temp) > 0:
				cmd.load(temp, name)
				cmd.load_traj(md, name, state=1, interval=10000)
	...
	for path in pdb_data:
		...
		if len(md) > 0 and len(temp) > 0:
			...
			cmd.color("green", f"{rep_name + forcefield} and resi 35-50 and name CA")
			cmd.set('all_states', 'on')
			cmd.ray(300, 300)
		obj = cmd.get_object_list('all')
		print(len(obj))
		for i in obj[1:]:
#			cmd.align(obj[0], i, object='aln', transform=0)
			cmd.align(f"{obj[0]} and resi 40", f"{i} and resi 35-50 and name CA", object='aln', transform=0)		
		cmd.center()
		cmd.zoom()
...
#subprocess.run(["python3", create_distance])
