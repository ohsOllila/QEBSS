...
def axs_plot(data_path, data_idx, column_nr, include_dev_points=False, include_header=True, axs=None, color_coord=True):
	...
	if ("R1") or ("R2" in y_header):
		axs.set_ylim(-y_max*0.1, y_max)
	if "Tau" in y_header:
		axs.set_yticklabels([f"{tick * 1e9:.1f}" for tick in axs.get_yticks()])
		axs.set_ylim(-y_max*0.1, y_max)	
...
def calculate_coil_probability(path, output, stride=20):
    ...
    plt.bar(range(len(average_coil_probabilities)), average_coil_probabilities, color='grey', width=1)
    plt.xlabel('Residue', fontsize=14)
    plt.ylabel('Coil Probability', fontsize=14)
    plt.gca().set_xticks(xticks)
    plt.gca().set_xticklabels([f"{i+1+412}" for i in xticks])
    #plt.gca().set_xticks(range(len(average_coil_probabilities)))
    #plt.gca().set_xticklabels([f"{i + 1 + 412} {residue_names[i]}" for i in range(len(average_coil_probabilities))], rotation=90, fontsize=6)
    plt.tight_layout()
...
def relaxation_combined(sim, exp, output, execution):
	...
	for ax3 in axs[-1]:
		...
		ax3.set_xticklabels([f"{i+1+412}" for i in xticks])
	for row in axs:
    ...
    for i, rep in enumerate(REPLICAS):
        for j, ff in enumerate(FORCEFIELDS):
            ...
            if os.path.exists(file):
                ...
                axs[i, j].set_yticklabels([f"{i+1+412}" for i in xticks])
                if rep + "/" + ff in Best_cases:
                    autoAxis = axs[i, j].axis()
                    rec = Rectangle((autoAxis[0]-(autoAxis[1] - autoAxis[0]) * 0.04, autoAxis[2]- (autoAxis[3] - autoAxis[2]) * 0.32), 
                        (autoAxis[1] - autoAxis[0])*1.08, 
                        (autoAxis[3] - autoAxis[2])*1.36, 
	...
	for path in data_path:
		...
		if same==False:
			...
			if name in Best_cases:
				autoAxis = axs[i, j].axis()
				rec = Rectangle((autoAxis[0]-(autoAxis[1] - autoAxis[0]) * 0.04, autoAxis[2]- (autoAxis[3] - autoAxis[2]) * 0.28), 
					(autoAxis[1] - autoAxis[0])*1.08, 
					(autoAxis[3] - autoAxis[2])*1.32, 
					fill=False, linewidth=5, edgecolor='black')
				axs[i, j].add_patch(rec)
				rec.set_clip_on(False)

			title_avg = str(round_sig(statistics.mean(rog_values))) + " nm"
			legend_item = Line2D([0], [0], color='w', lw=0, label=f'Avg = $\\mathbf{{{str(title_avg)}}}$')
