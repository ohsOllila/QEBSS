...
PARAM_DIR=/scratch/project_462000154/cmcajsa/${FORCEFIELD}
sim_time=sim_time
...
if [[ "${ions1[*]}" =~ "${i}" ]]; then
	echo SOL | srun -n 1 gmx_mpi -quiet genion -s ions_${SUFFIX}_${i}.tpr -o ${PROTEIN}_solv_ions_${i}.gro -p topol.top  -pname ${i} -nname CL -neutral -conc 1  
else
	echo SOL | srun -n 1 gmx_mpi -quiet genion -s ions_${SUFFIX}_${i}.tpr -o ${PROTEIN}_solv_ions_${i}.gro -p topol.top  -pname ${i} -pq 2 -nname CL -neutral -conc 1 
fi
...
if [ -f md*${sim_time}ns.gro ]; then
	echo "GRO file already exists, skipping simulation"
elif [ -f md*${sim_time}ns.xtc ] && [ ! -f md*${sim_time}ns.gro ]; then 	
